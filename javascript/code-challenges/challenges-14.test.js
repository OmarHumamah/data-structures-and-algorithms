...
let biggerThanLuke = (arr) => {
  if (arr.length === 0) return "";
    ...
    let newArr = [];
    arr.forEach (n=> {
      if (parseInt(n.mass) > parseInt(arr[0].mass)) {
        newArr.push(n.name);
      }
    })
...
const sortBy = (property, arr) => {
  ...
  if (property == "name") {
   ...
   newArr = arr.sort((a, b) => {
      ...
      if ( parseInt(a.price) > parseInt(b.name)){
        return 1;
      }
      return 0;
    });
...
const detectTicTacToeWin = (board) =>{
 ...
 let checkHorizontally = (arr) => {
  ...
  let checkDiagonally = (arr) =>{
    ...
    if(strArr.join('')==='XXX' ||strArr.join('')==='OOO' ||strArr2.join('')==='XXX' ||strArr2.join('')==='OOO')
    {
      check = true;
    }
    return check;
  ...
  return checkHorizontally(board)||checkVertically(board)||checkDiagonally(board)
};
/* ------------------------------------------------------------------------------------------------
TESTS
