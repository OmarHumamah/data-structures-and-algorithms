...
const sortByChildren = (charArray) => {
  return charArray.sort(
    ({ children, house }, { children: secChildren, house: secHouse }) => {
      if (children.length === secChildren.length) return house > secHouse;
      else return children.length - secChildren.length;
    }
  );
};
...
const containsWorld = (input) => {
  const regex = new RegExp(`world`, "gm");
  regex;
  return regex.test(input);
};
...
const citiesAtoJ = (arr) => {
  return (arr.map((a) => a.replace(/ * /gm, "")).join("\n").match(/^[A-J]\w*/gm) || []);
};
