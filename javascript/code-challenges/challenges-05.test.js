...
const toLastNames = people => {
  let newArray = people.map(n => n.firstName +' '+ n.lastName)
  return newArray
};
...
const addValues = (arr) => {
   let newArray = arr.reduce((acc, val)=>{ acc = acc + val; return acc}, 0)
  return newArray
};
...
const addPurchases = (arr) => {
  let total = arr.reduce((acc,val)=>{ acc = acc + val.purchasePrice; return acc},0)
  return total
};
...
const countNumberOfElements = (arr) => {
  let itemTotal = arr.reduce((acc, val, i)=>{ acc = i + 1 ;return acc},0)
  return itemTotal
};
...
const returnNames = (arr) => {
  let names = arr.reduce((acc,val,i)=>{acc[i] =  val.name ; return acc },[])
  return names
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const reversedString = (str) => {
  let arr = str.split('');
  let reversedStr =  arr.reduce((acc, val) => val + acc,"");
  return reversedStr
};  
