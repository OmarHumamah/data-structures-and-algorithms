...
let findMaleAndFemale = (data) => {
  let mAndF = data.filter(n=>{
    if(n.gender === 'female' || n.gender === 'male'){
    return n}
  }).map(n => n.name ).join(' and ')
  return mAndF
};
...
let findShortest = (data) => {
  let name = data.reduce(((x,y)=>{
    y.height > x.height ? x = y.name : false
    return x
  }))
return name
};
